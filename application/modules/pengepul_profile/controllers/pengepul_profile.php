<?php
class pengepul_profile extends pengepul_controller{
    var $controller;
    function pengepul_profile(){
    ...
    function simpan(){
        ...
        $this->form_validation->set_rules('username','Cek Username','callback_cek_username');
        ...
        if($this->form_validation->run() == TRUE ) {  // validasi berhasil 
            ...
            $login = $this->session->userdata("pengepul_login");
            $user_id = $login['id'];

            $this->db->where('id', $user_id);
            $res = $this->db->update("super_admin",$data);
            if(!$res){
